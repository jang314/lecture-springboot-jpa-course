...
//                .fetchFirst();
        ...
        // Query가 두번 실행 됨. total count를 가져오기 때문이다.
//        QueryResults<Member> results = queryFactory.selectFrom(member)
//                .fetchResults();
//        long count = results.getTotal();
//        List<Member> content = results.getResults();

        // select절을 카운트 쿼리로 바꿔준다.
    ...
    * 예) 회원과 팀을 조인하면서,팀 이름이 teamA인 팀만 조인, 회원은 모두 조인
     * JPQL : select m, t from Member m left join m.team t on tm.name= 'teamA'
    */
    @Test
...
//    }
    ...
    void join_on_no_relation() {
        ...
        List<Tuple> result = queryFactory
                .select(member, team)
                .from(member)
                .leftJoin(team).on(member.username.eq(team.name)) // id를 매칭을 안하고 name으로만 필터링됨 (leftJoin에 member도 넣으면 id 조인까지 함)
                .fetch();
    ...
    public void fetchJoinNo() {
        ...
        //  이미로딩된 엔티티인지 아니면 초기화가 안된 엔티티인지 알려줌
    ...
    void selectSubQuery() {
        QMember memberSub = new QMember("memberSub");
        List<Tuple> tuple = queryFactory.select(member.username,
                        select(memberSub.age.avg())
                                .from(memberSub)
                )
                .from(member)
